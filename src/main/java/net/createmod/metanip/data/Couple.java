...
// Removed: Codec, StreamCodec, NBTHelper, CompoundTag, ListTag (not available in 1.7.10)
...
public class Couple<T> extends Pair<T, T> implements Iterable<T> {
    ...
    public T get(boolean first) { return first ? getFirst() : getSecond(); }

    public void set(boolean first, T value) {
        if (first) setFirst(value);
    ...
    public <S> Couple<S> mapNotNull(Function<T, S> function) {
        return Couple.create(first != null ? function.apply(first) : null,
    ...
    public Couple<T> swap() { return Couple.create(second, first); }

    @Override
    public Iterator<T> iterator() { return new Couplerator<>(this); }
    ...
    private static class Couplerator<T> implements Iterator<T> {
        ...
        @Override
        public boolean hasNext() { return state != 2; }

        @Override
        public T next() {
            state++;
            if (state == 1) return couple.first;
            if (state == 2) return couple.second;
            return null;
        }
    }
}
