...
// Pure Java - only needed Java 8 switch syntax fix (no arrow cases)
public class Glob {
    private static final String REGEX_META_CHARS = ".^$+{[]|()";
    private static final String GLOB_META_CHARS = "\\*?[{";
    private static final char EOL = 0;

    public static boolean isRegexMeta(char c) { return REGEX_META_CHARS.indexOf(c) != -1; }
    public static boolean isGlobMeta(char c)  { return GLOB_META_CHARS.indexOf(c) != -1; }
    ...
    public static String toRegexPattern(String globPattern) throws PatternSyntaxException {
        ...
        while (i < globPattern.length()) {
            ...
            if (c == '*') {
                ...
                else { regex.append(','); isAnchored = true; }
            } else if (c == '[') {
                ...
                if (next(globPattern, i) == '^') {
                    ...
                    if (next(globPattern, i) == '!') { regex.append('^'); ++i; }
                    if (next(globPattern, i) == '-') { regex.append('-'); ++i; }
                }
                ...
                if (inGroup) {
                    regex.append(")");
                    if (isAnchored && isNegativeLookaround) { regex.append(".*"); isAnchored = false; }
                    inGroup = false;
                } else { regex.append('}'); isAnchored = true; }
            } else {
